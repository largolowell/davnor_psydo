...
export class ResdavnorgymComponent implements OnInit {
  ...
  validateInput(){
    ...
    if(this.validate.purpose || this.validate.eventDate || this.validate.startTime || this.validate.endTime){
  ...
  total(){
    ...
    this.addData.total = this.rate * this.totalTime + (this.rateOther * this.totalTime);
  }
